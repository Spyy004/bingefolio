...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<int> getLikesOfAPost(portfolioId)async{
  ...
  Future<bool> likePortfolio(portfolioId, likes) async {
    if (FirebaseAuth.instance.currentUser == null) {
      genericLoginPopup(
          "You need to login or signup to Bingefolio before liking a portfolio. Get onboard now!");
      return false;
    }
    final entries = await db
        .collection("likes")
        ...
        .get();
    if (entries.count! > 0) {
      showGenericToast("Post already liked");
      return false;
    }
    await db
        .collection("likes")
        .doc( FirebaseAuth.instance.currentUser!.uid + portfolioId.toString())
        .set({"portfolioId": portfolioId, "userId": FirebaseAuth.instance.currentUser!.uid});
    final totalLikesOnServer = await db
        .collection("likes")
        .where("portfolioId", isEqualTo: portfolioId)
        .count()
        .get();

    db.collection('portfolios').doc(portfolioId).update({"likes": totalLikesOnServer.count!});
      showGenericToast("Portfolio liked successfully");
  ...
  Future<List<Map<String, dynamic>>> getPortfolios() async {
    ...
    for (var docSnapshot in querySnapshot.docs) {
      if (selectedDevFilter != 0 && selectedPortfolioFilter != 0) {
        ...
        if (docSnapshot
                .data()
                .containsValue(filterDeveloperType[selectedDevFilter]) &&
            docSnapshot
                .data()
                .containsValue(filterTechStacks[selectedPortfolioFilter])) {
    ...
    portfolios = portfolios
        .where((element) => element['name'].toString().contains(searchQuery))
        .toList();
    return portfolios;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          genericLoginPopup(
              "You need to login or signup to Bingefolio before adding your portfolio. Get onboard now!");
      ...
      child: SingleChildScrollView(
        child: Container(
          child: Column(
            ...
            children: [
              Row(mainAxisAlignment:  currentUser != null ? MainAxisAlignment.end : MainAxisAlignment.center, children: [
                Center(
                  child: RichText(
                      text: TextSpan(
                          children: [
            ...
            currentUser != null?  Padding(
               padding:  EdgeInsets.only(left: MediaQuery.of(context).size.width * 0.3, right: 16),
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (snapshot.connectionState ==
                            ...
                            if (snapshot.data!.length == 0) {
                              return const Center(
                                child: Text(
                                    "No Porfolios found for this filter. Why don't you create one?"),
                              );
                            }
                            return Padding(
                          ...
                          return Container(
                            child: const Center(
                                child: Text(
                                    "Sorry we ran into a problem. We will be back again")),
                          );
                        }),
                  ),
                ],
                ...
                onPressed: () async {
                  String portId =
                      widget.snapshot.data![widget.index]['name'].toString() +
                          widget.snapshot.data![widget.index]['developerType']
                              .toString();
